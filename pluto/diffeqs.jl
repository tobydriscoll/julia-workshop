...
# ╔═╡ e15e4540-50af-47af-9874-97afc86e9e55
using DiffEqParamEstim

# ╔═╡ 55029043-8b17-477e-8a47-e610590cca08
using Optim

# ╔═╡ 6b2b81a3-c95a-4ac8-94cf-2bda8353b984
using DiffEqBayes,Distributions

# ╔═╡ 44ee20de-72b2-4b8b-9588-ccb84ad3bc25
using StatsBase

# ╔═╡ e237996a-2a9e-4835-be13-74b37bebe541
md"""
The `DifferentialEquations` package is massive and changes frequently. You will refer often to the [documentation](https://diffeq.sciml.ai/stable).
"""

# ╔═╡ df55dce4-621f-4289-b871-d47bd1f979b6
md"""
## Basic IVP solutions

Let's solve the classic Lotka–Volterra equations,

```math
\begin{split}
\frac{dx}{dt} &= \alpha x - \gamma xy, \\
\frac{dy}{dt} &= -\beta y + \delta xy. 
\end{split}
```
"""
...
function predprey(u,p,t)
	x,y = u      # current solution values
	α,β,γ,δ = p  # parameter values
	dx_dt = α*x - γ*x*y
	dy_dt = -β*y + δ*x*y
	return [dx_dt,dy_dt]
end
...
begin
	u0 = [40.,3.]
	tspan = (0.,200.)
	pp_ivp = ODEProblem(predprey,u0,tspan,[0.2,0.3,0.02,0.05])
end
...
let
	pp_sol = solve(remake(pp_ivp,tspan=(0.,60.)),saveat=0.2)
	animate(pp_sol,vars=(1,2))
end
...
Each call to evaluate the time derivative requires the creation of a 2-vector for the result. The time this takes can be saved by using the *in-place* form of the function, which mutates the first input argument instead.
"""

# ╔═╡ 42801b27-b74a-4721-9332-1cf7ac81425d
function predprey!(du_dt,u,p,t)
	x,y = u
	α,β,γ,δ = p
	dx_dt = α*x - γ*x*y
	dy_dt = -β*y + δ*x*y
...
# ╔═╡ 8f0d4489-d3dd-4b19-a4e3-df7bee5cf69f
md"""
## Choosing the integrator

If `solve` is not supplied with any directions, it chooses an integrator based on heuristics. You can give hints about the problem, or select an integrator manually.
"""

# ╔═╡ 295f7ad0-62bd-44b3-a66a-4aa7f7c50b52
let
	sol = solve(pp_ivp,reltol=1e-5,abstol=1e-5,alg_hints=[:nonstiff])
	sol.alg,length(sol.t)
end

# ╔═╡ 2fbb0f33-ba04-492e-ae6b-d838930d9e38
let
	sol = solve(pp_ivp,reltol=1e-5,abstol=1e-5,alg_hints=[:stiff])
	sol.alg,length(sol.t)
end

# ╔═╡ 24c1a2bf-c9b0-40cd-9cd6-95e73386c531
let
	sol = solve(pp_ivp,reltol=1e-12,abstol=1e-12,alg_hints=[:nonstiff])
	sol.alg,length(sol.t)
end

# ╔═╡ 32374522-03f0-4a9a-ae54-c7cff45587ef
let
	sol = solve(pp_ivp,DP5(),reltol=1e-12,abstol=1e-12)
	sol.alg,length(sol.t)
end

# ╔═╡ 611c56a4-3ebe-465e-8679-0257ce150e0f
md"If you've got your heart set on a particular method, there is a [long list](https://diffeq.sciml.ai/stable/solvers/ode_solve/) of them to choose from."

# ╔═╡ a7517a6d-6fb9-4eb2-bed4-dd04e9d56254
md"You can use the native MATLAB algorithms, though there is not much reason to do so."

# ╔═╡ 9982199f-32f9-4bd9-9077-f444081d4259
md"""
## Sensitivity analysis

Most problems have parameters that remain fixed during each instance. It may be valuable to know how sensitive the solution is to variation in these parameters. There are many definitions and methods for this analysis, but the simplest conceptually is to differentiate the solution with respect to the parameters.
"""

# ╔═╡ 134ace87-de0e-4816-a88a-6d8ed3f7ea50
begin
	pp_sense = ODEForwardSensitivityProblem(predprey!,u0,tspan,[0.2,0.3,0.02,0.05])
	pp_sense_sol = solve(pp_sense,saveat=0.25)
	u,du = extract_local_sensitivities(pp_sense_sol)
	size(du)
end
...
# ╔═╡ 25df1ce2-055c-4197-9f28-1094c55317a4
size(du[1]')

# ╔═╡ 48600237-d514-4d6e-b0bd-5b9e5598343b
md"""
## Parameter identification

We are often faced with the inverse problem of determining parameters from (usually noisy) observations of the system. There are a few ways to do this in add-on packages.
"""

# ╔═╡ ea18fe1e-bb22-4ddc-8749-804b7c493003
md"""
The best-known and conceptually simplest method is to minimize some measure of misfit between simulations and observations. 

To demonstrate, we first generate some artificial data from known parameter values.
"""

# ╔═╡ a8bbc79e-d4d2-4938-aadb-cf241100e1c7
begin
	p_nom = [0.12,0.04,0.01,0.003]
	ivp_nom = ODEProblem(predprey!,u0,tspan,p_nom)
	sol_nom = solve(ivp_nom)
	nominal = (p=p_nom,ivp=ivp_nom,sol=sol_nom)
	t_obs = range(tspan...,81)
	u_obs = [ max.(0,sol_nom(t) + 4randn(2)) for t in t_obs ]
	observed = (t=t_obs,u=hcat(u_obs...))
	observed.u[:,1:6]
end

# ╔═╡ 6d178345-4dd4-417e-b672-64c10b67d689
let
	opts = Dict(:label=>["prey" "predator"])
	plot(nominal.sol;opts...)
	scatter!(observed.t,observed.u';m=3,opts...)
end

# ╔═╡ 86da2dfb-d293-4476-8949-37a101dc2709
md"We use a utility to build a function that evaluates the sum-of-squares error between simulation and observations."

# ╔═╡ 9fea8298-54e5-4b51-a8a6-12fdeaf0dcad
misfit = build_loss_objective(nominal.ivp,Tsit5(),L2Loss(observed.t,observed.u),
                                     maxiters=1000,verbose=false);

# ╔═╡ 6b4e502c-0463-4586-8e3c-6fe30237e391
md"We can now use any optimization method we like. Here, we choose `Optim`."

# ╔═╡ 476dde85-5c4b-4cfb-b347-624312723890
bestfit = optimize(misfit,[.05,.05,.01,.01])

# ╔═╡ b01607d2-fc69-49db-a3ea-e0ab8250e9d8
bestfit.minimizer

# ╔═╡ 5e3f15c5-8008-4305-a586-097b19c394da
nominal.ivp.p

# ╔═╡ 6a6f6c7b-ef47-4e67-b4ae-633f74752c3e
let
	best = solve( remake(nominal.ivp,p=bestfit.minimizer) )
	plot!(best,label=["est. prey" "est. predator"])
end

# ╔═╡ ae109ba3-baa0-4af3-9e08-df21815df318
md"""
A more nuanced approach is to specify prior beliefs about the parameters and then update those beliefs using Bayesian analysis on the observations. The result has probability distributions for the parameters, not just best values.
"""

# ╔═╡ 5c0b848c-85fb-4175-aca3-33a5001bc852
priors = (
	truncated( Distributions.LogNormal(-2,1), 0,0.2 ),
	truncated( Distributions.LogNormal(-2,1), 0,0.2 ),
	truncated( Distributions.LogNormal(-4,0.5), 0,0.1 ),
	truncated( Distributions.LogNormal(-4,0.5), 0,0.1 ),
)

# ╔═╡ ece4f71b-dfa5-4380-b9ce-397cdf2f95ae
let
	plot(x->pdf(priors[1],x),0,0.2,layout=(2,1),subplot=1,
		label="α,β",plot_title="Priors")
	plot!(x->pdf(priors[3],x),0,0.1,subplot=2,label="γ,δ")
end


# ╔═╡ 28e3aaa2-2ada-469d-8972-3ebaee513275
syms = [:α,:β,:γ,:δ];

# ╔═╡ d89a6276-1fd2-41e3-b911-5e881cae790c
bayes = turing_inference(nominal.ivp,Tsit5(),observed.t,observed.u,priors,
		num_samples=1000,syms=syms)

# ╔═╡ f0998c04-3424-4bc4-a346-83acef8b5c4f
bayes[syms[1]]

# ╔═╡ 9776916c-9e4d-4d8f-b7ad-6cae32f2f410
describe(bayes)

# ╔═╡ 653c2944-ceca-45b1-8731-143f96b0692b
nominal.ivp.p

# ╔═╡ 6b391d21-25ba-4959-bb27-f933d3674ecb
let
	plt = plot(size=(600,400),layout=(4,1))
	for (s,var) in enumerate(syms)
		histogram!(bayes[var],subplot=s,label=string(var),normalize=:pdf)
		vline!([nominal.ivp.p[s]],subplot=s,label="nominal",color=:red)
	end
	plt
end
...
manifest_format = "2.0"
project_hash = "fb4e1fa74bf8dda8fff9c3df0b42531f47297a05"
...
git-tree-sha1 = "7c17ffd0edca6fff9045c038c8af565535f26e46"
...
git-tree-sha1 = "ee2c300e12e40f10850ab7dfbacabc12abcd17f2"
...
git-tree-sha1 = "133000825e48d4f39d2ff058125c8be5c1662eb1"
